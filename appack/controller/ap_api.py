...
import requests
import logging
from appack.conf.conf_url import *
...
class apApi:
    ...
    def urlencode(self,paraDict):
        self.para = urllib.parse.urlencode(paraDict)
        return self.para
    ...
    def getPara(self,urlEncode=False,**kwargs):
        para = kwargs
        ...
        log.debug(para)
        if urlEncode:
            self.para = self.urlencode(para)
        else:
            self.para = para
        return self.para

    def getParaDict(self,para,urlEncode=False):
        para['_time'] = int(time.time())
        para['_appname'] = 'papi'
        sign = getSign(para)
        para['_sign'] = sign
    ...
    def get(self,para={},urlEncode=False,**kwargs):
        try:
            if para:
                self.getParaDict(para,urlEncode)
            else:
                self.getPara(urlEncode,**kwargs)
    ...
    def add_oem(self,**kwargs):
        ...
        res = self.get(urlEncode=True,**kwargs)
        return res
    ...
    def release_pg(self,para):
        ...
        res=self.get(para,urlEncode=True)
        return res
