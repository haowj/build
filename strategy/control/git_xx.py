...
from strategy.base.op_db import insert_into_data, update_data_base_one
log = logging.getLogger(__name__)


def delete_super_tag(**kwargs):
    """
    git 插件数据删除
    软删除  status == 0 为删除状态
    :param kwargs:
    :return:
    """
    data = update_data_base_one('S_O_T_DB', 0, **kwargs)
    if type(data) == str:
        log.warning(f'git 插件数据删除失败，原因 {data}')
    else:
        log.debug(f'git 插件数据删除成功 {kwargs}')
...
class GitProcess:
    ...
    def createTag(self,tag,msg,sw,path,branch,user=''):
        ...
        try:
            ...
            inset_into_super_tag(project_path=path,
                                 comment=msg,
                                 user=user,
                                 project_name=sw,
                                 tag=tag)
    ...
    def delTag(self,tag,sw):
        ...
        for cmd in [cmd_r,cmd_l]:
            ...
            rdata.append(data)
        delete_super_tag(project_name=sw,
                         tag=tag)
        return rdata